...
As a non-distributed algorithm was applied to the input data, figure
\ref{fig:algo:use} shows a round-robin like distribution of tasks for \ac{JSQ}.
Surprising is that ``RS-JSQ'' shows almost the same distribution, though its
random component.

Figure \ref{fig:algo:load} shows the distribution of the load in the system.
The algorithms did not know about the load, which was assigned randomly to the
tasks.
Anyways, it is clearly visible that ``random'' distribution of tasks does result
in a worse distribution than ``roundrobin'' or \ac{JSQ}, given the default
deviation as shown in \ref{fig:algo:load:defdev}.
